...
- 归档操作：先跑 `pnpm exec tsx scripts/check-doc-references.ts` 确认引用不断裂，再 `git mv` 到 `docs/archived/archived-plans/`，同步更新 `docs/archived/README.md` 归档表和 `docs/todos/README.md` 索引
...
- 最小验证：按改动范围运行对应包级测试；涉及 runtime/profile/route surface 时补 `pnpm test:deployment-smoke` 或 `pnpm test:runtime-foundations`
- 必须同步：API surface、运行时默认值、健康检查、部署行为变化时，更新对应 `reference/`、`architecture/`、`operations/` 文档
- 涉及 service zone 内部模块之间的跨包导入重构时，运行 `pnpm exec fallow list --boundaries` 确认当前 zone 覆盖范围
...
- 最小验证：`pnpm --filter @trapmap/contracts test --run <test-path>`；再跑受影响包的最小测试与 `pnpm typecheck`
...
- 最小验证：受影响测试 + `pnpm typecheck`；涉及 runtime/env surface 时补 `pnpm test:deployment-smoke`
...
- 最小验证：`pnpm check:docs`、`pnpm check:structure`；必要时补对应 truth smoke
...
- 最小验证：`pnpm test:observability-closeout`、`pnpm test:discovery-closeout`；涉及 distributed hop 补 `pnpm test:distributed-closeout`；涉及部署配置时补 `pnpm test:deployment-smoke` 与 `pnpm test:runtime-foundations`
