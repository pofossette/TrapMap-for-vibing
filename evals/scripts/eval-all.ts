...
async function loadSummaryCases(
  options: EvalAllOptions,
  endpoint?: SummaryEvalCase['endpoint'],
): Promise<SummaryEvalCase[]> {
  const { getSummaryEvaluationCases } = await import('../summary/lib/runner-api.js');
...
async function buildSuitePlatformEvents(
  ...
  if (summaryResult?.report) {
    ...
    const suiteRunIdWithSuffix = `${suiteRunId}:summary`;
    const summaryCases = await loadSummaryCases(options, report.meta.options.endpoint);
    const summaryCaseMap = new Map(summaryCases.map((case_) => [case_.caseId, case_]));
    const summaryFailuresByCase = groupSummaryFailuresByCase(report.failures);
    events.push({
      family: 'EvalRunStarted',
      suite: 'summary',
      tier: report.meta.options.tier,
    ...
    events.push(
      ...buildSummaryCasePlatformEvents({
        ...
        baseTags: tags,
        report,
        caseMap: summaryCaseMap,
        failuresByCase: summaryFailuresByCase,
      }),
    );
    events.push({
      family: 'EvalRunFinished',
