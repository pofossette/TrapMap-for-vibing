...
要做的内容：
- [ ] 审计当前所有持久化路径，确认哪些模块仍依赖 `store_snapshot`、`SkillShareerStore`、`InMemory*Repository` 或双写兼容层。
- [ ] 盘点所有核心表、运行时建表逻辑、现有 migration、索引、序列、统计表与检索索引表。
- [ ] 按领域拆分目标模型：知识、技能工件、候选、反馈、统计、检索索引、团队与用户引用。
- [ ] 明确哪些 `JSONB` 必须拆分，哪些允许保留为低频扩展字段。
- [ ] 明确每个领域的“唯一事实源表”“历史表”“事件表”“派生索引表”。
- [ ] 明确迁移策略：一次性切换字段、分批回填、影子校验、停止双写、删除旧层。
- [ ] 确定命名规范：主键、业务 ID、外键、索引、约束、时间字段、状态字段命名统一。
