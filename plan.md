...
- **目标：** 让文档事实可由源码验证，并将 OTel 收敛为统一、可运营的遥测标准，以可选 Sentry 异常智能层补足错误聚合。
- **状态：** `进行中`
...
| 4. 运营闭环 | Task 9-10 | 可选 Sentry 脱敏适配器、live verification、runbook 和 deferred gates | telemetry 故障不影响业务，所有完成门禁都有可复核证据 |
...
- 可选的 Sentry actionable-error 适配器，以及无密钥的运营验证、runbook 和采用门槛。
...
- `backend-core` 或领域包直接依赖 OTel/Sentry SDK，或引入第二条 trace/metrics 管线；
