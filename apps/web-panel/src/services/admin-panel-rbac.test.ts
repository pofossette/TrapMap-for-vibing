...
import { isUnauthorizedError } from './admin-panel-service-context';
...
describe('real admin RBAC — 401 handling', () => {
  ...
  it('withAuthRedirect wraps real api to clear session on 401 and rethrows', async () => {
    ...
    const { isUnauthorizedError: check } = await import('./admin-panel-service-context.js');
    ...
    const { isUnauthorizedError } = await import('./admin-panel-service-context.js');
