...
export function registerRegistryCommands(program: Command): void {
  const registry = program.command('registry').description('Skill package registry (skills.sh / GitHub / ai-pkgs / local)');

  // trapmap skill add <source> — mirrors ai-pkgs skills add + ccswitch install
  registry
    .command('add')
    .argument('<source>', 'Skill source: owner/repo, skills.sh/<slug>, github:owner/repo, ./local-path')
    ...
    .action(async (source: string, opts: { agent: string; global: boolean; json: boolean }) => {
      ...
      const agents = opts.agent === 'all' ? ['claude-code','codex','cursor','trapmap'] : [opts.agent];
      const result = await installer.install(source, { scope: opts.global ? 'global' : 'project', agentTargets: agents });
      if (opts.json) console.log(JSON.stringify(result, null, 2));
      else console.log(`Installed ${result.slug}@${result.version ?? 'unknown'} -> ${result.installedPath} (${result.filesWritten} files)`);
    });
  ...
  registry
    ...
    .action(async (query: string, opts: { limit: string; json: boolean }) => {
      ...
      if (opts.json) console.log(JSON.stringify(all, null, 2));
        ...
        for (const r of all) {
          console.log(`\n[${r.registry}]`);
          for (const e of r.entries) console.log(`  - ${e.slug} ${e.latestVersion ?? ''} — ${e.description ?? ''}`);
        }
  ...
  registry
    ...
    .action(async (opts: { global: boolean; json: boolean }) => {
      ...
      const lockPath = opts.global ? path.join(process.env.HOME ?? process.cwd(), '.trapmap', 'skills.lock') : path.join(process.cwd(), '.trapmap', 'skills.lock');
      try {
        ...
        else for (const e of entries) console.log(`${e.slug}@${e.version} (${e.scope}) ${e.source.canonical}`);
  ...
  registry
    ...
    .action(async (slug: string | undefined, opts: { global: boolean; json: boolean }) => {
      ...
      const results = slug ? [await updater.update(slug, { scope })] : await updater.updateAll(process.cwd(), scope);
      if (opts.json) console.log(JSON.stringify(results, null, 2));
      else for (const r of results) console.log(r.updated ? `Updated ${r.slug} ${r.from} -> ${r.to}` : `Skip ${r.slug}: ${r.reason ?? 'up to date'}`);
    });
  ...
  registry
    ...
    .action(async (opts: { global: boolean; json: boolean }) => {
      ...
      if (opts.json) console.log(JSON.stringify(result, null, 2));
      else for (const r of result) console.log(`${r.slug}: ${r.hasLocalEdits ? 'modified' : 'clean'}`);
    });
}
...
export function registerSkillAddTopLevel(skill: Command): void {
  skill
    ...
    .action(async (source: string, opts: { agent: string; global: boolean; json: boolean }) => {
      ...
      const result = await installer.install(source, { scope: opts.global ? 'global' : 'project', agentTargets: agents });
      if (opts.json) console.log(JSON.stringify(result, null, 2));
      else console.log(`Installed ${result.slug}@${result.version ?? 'unknown'} -> ${result.installedPath}`);
    });
}
