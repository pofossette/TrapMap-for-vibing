...
- [数据库表结构速查](reference/DATABASE_SCHEMA.md) — 65 张表快速参考、枚举值、外键关系
...
- [根计划索引](../plan.md) — 当前链接唯一 active mainline 为 assembly Phase 1 地基，并保留归档、debt 与背景入口
- [assembly Phase 1 细则](todos/assembly-phase1.md) — 当前 active execution surface（统一优雅组装中心）；[死代码与架构秩序清理](todos/dead-code-and-architecture-order-cleanup.md) 为上一主线细则，实现已提交 2026-08-16、closeout 延后（见 [open-debt](todos/open-debt-and-compromises.md)）
- 当前唯一 active execution surface 是 assembly Phase 1 地基；新的 todo 文档必须先由根 `plan.md` 显式赋予执行责任，其余材料只能作为背景输入、deferred 落点或已完成 closeout 参考
...
- [数据库表结构速查](reference/DATABASE_SCHEMA.md) — PostgreSQL 65 张表完整参考
