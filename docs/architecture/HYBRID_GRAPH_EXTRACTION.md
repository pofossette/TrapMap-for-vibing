...
| 阶段 | 状态 | 预估 | 开始 | 完成 | 备注 |
|------|------|------|------|------|------|
| Phase 1 | ⬜ 未开始 | 4-5 天 | - | - | 图实体提取 LLM 化（基础层） |
| Phase 2 | ⬜ 未开始 | 2-3 天 | - | - | 重复/冲突检测增强（可与 P3 并行） |
| Phase 3 | ⬜ 未开始 | 2 天 | - | - | Pre-review 增强（可与 P2 并行） |
| Phase 4 | ⬜ 未开始 | 2-3 天 | - | - | 缓存与评测框架 |

### 详细进度

#### Phase 1: 图实体提取 LLM 化
- [ ] 1-1: Zod Schema 定义 (0/8)
- [ ] 1-2: Prompt 模板 (0/6)
- [ ] 1-3: LLM 提取核心模块 (0/12)
- [ ] 1-4: Gleaning 二次提取 (0/4)
- [ ] 1-5: 图适配器集成 — Trap 侧 (0/6)
- [ ] 1-6: 图适配器集成 — Skill 侧 (0/4)

#### Phase 2: 重复/冲突检测 LLM 增强
- [ ] 2-1: LLM 重复判定模块 (0/6)
- [ ] 2-2: 集成到 detector.ts (0/7)
- [ ] 2-3: LLM 冲突判定模块 (0/5)
- [ ] 2-4: 集成到 detect.ts (0/4)

#### Phase 3: Pre-review LLM 增强
- [ ] 3-1: 扩展 boundary-extract.ts (0/6)
- [ ] 3-2: 修改 pre-review.ts (0/6)
- [ ] 3-3: Prompt 优化 (0/3)
