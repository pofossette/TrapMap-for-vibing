...
- originalRole: `root-control-plan`
- archivedReason: `Replaced by the backend engineering master plan as the active root execution track`
- replacementEntry: [`plan.md`](../../../plan.md)

## 状态

- 状态：`archived`
- 原目标日期：`2026-06-18`
- 本文件原角色：根级总计划与索引，只描述背景、总要求、阶段边界与子计划入口
...
- [00-baseline-and-target-architecture.md](../../plans/runtime-recomposition/00-baseline-and-target-architecture.md)
  作用：冻结术语、边界、目标拓扑、包布局和非目标，给后续子计划提供统一前提。

### B. 共享客户端核心包

- [01-shared-client-core-extraction.md](../../plans/runtime-recomposition/01-shared-client-core-extraction.md)
  作用：规划从 `packages/cli/src/lib/http.ts` 和通用逻辑中抽出新包，定义 Web 面板与 CLI 的共享 API。

### C. 后端公共核心内核

- [02-backend-core-kernel-extraction.md](../../plans/runtime-recomposition/02-backend-core-kernel-extraction.md)
  作用：规划把 `packages/server` 中与宿主无关的应用编排、端口、能力模型、以及更细粒度服务单元边界上提为核心内核。

### D. 轻量本地宿主

- [03-light-host-assembly.md](../../plans/runtime-recomposition/03-light-host-assembly.md)
  作用：规划本地/单实例轻宿主，服务 `local-agent` 与 `team-monolith` 的最小装配方案。

### E. 重型微服务宿主

- [04-heavy-microservice-assembly.md](../../plans/runtime-recomposition/04-heavy-microservice-assembly.md)
...
- [05-migration-validation-and-doc-rollout.md](../../plans/runtime-recomposition/05-migration-validation-and-doc-rollout.md)
