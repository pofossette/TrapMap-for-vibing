...
- `packages/host-local/`: Lightweight host assembly for local-agent and team-monolith profiles. `src/nest/**` is the Phase 4 default modular-monolith mainline and default development entry; the six bounded-context Nest modules (`identity-access/`, `knowledge-read/`, `knowledge-write/`, `candidate-ingestion/`, `governance-review/`, `job-runtime/`) are registered in `src/nest/app.module.ts` and share the same module graph across profiles; legacy Fastify paths (`src/bootstrap/**`, `src/http/**`, `src/runtime/**`) remain only as rollback/compatibility shell during the migration window and should keep shrinking.
