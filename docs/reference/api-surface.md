...
>
> 契约落点：`/health`、`/live`、`/ready`、`/metrics` 无请求/响应 contracts schema（runtime 面，不适用）。
...
> 契约落点：`POST /v1/candidates` 请求 `candidateSubmissionRequestSchema`（packages/contracts/src/domain/candidates.ts:299）／响应 `candidateSubmissionResponseSchema`（packages/contracts/src/domain/candidates.ts:312）；`GET /v1/candidates` 请求 `candidateListSchema`（网关本地 `packages/host-distributed/src/gateway/route-defs/shared.ts:411`）／响应 `candidateListResponseSchema`（packages/contracts/src/domain/candidates.ts:326）；`GET /v1/candidates/:candidateId` 请求无／响应 `candidateStatusResponseSchema`（packages/contracts/src/domain/candidates.ts:320）；`POST /v1/candidates/:candidateId/resolution` 请求 `candidateResolutionSchema`（网关本地 `packages/host-distributed/src/gateway/route-defs/shared.ts:417`）／响应 `applyResolutionResponseSchema`（packages/contracts/src/domain/candidates.ts:425）；`POST /v1/candidates/:candidateId/manual-result` 请求 `ManualResultSubmissionSchema`（packages/contracts/src/domain/candidates.ts:366）／响应 `manualResultResponseSchema`（packages/contracts/src/domain/candidates.ts:445）；light 侧 `POST /v1/candidates/:candidateId/apply-resolution` 请求无／响应 `applyResolutionResponseSchema`（packages/contracts/src/domain/candidates.ts:425）。
...
> 契约落点：`POST /v1/jobs` 请求 `scheduleJobSchema`（网关本地 `packages/host-distributed/src/gateway/route-defs/shared.ts:599`）／响应（未收敛，2026-09-08）；`GET /v1/jobs/:jobId` 请求无／响应 `{ status: 'pending' | 'running' | 'completed' | 'failed' | 'dead'; result?: unknown; error?: string }`（`packages/backend-core/src/ports/internal-ports.ts:330`）；`GET /v1/jobs/queue` 请求无／响应 `{ pending: number; running: number; dead: number }`（`packages/backend-core/src/ports/internal-ports.ts:339`）；`GET /v1/operations/status/async` 请求无／响应网关内联 envelope（`packages/host-distributed/src/gateway/route-defs/job.ts:70`，含 `asyncRuntimeEnabled`、`queue`、`outbox`、`retryResumeContract`）。
...
> 契约落点：`GET /v1/cron/jobs` 请求无／响应 `cronJobListSchema`（light 网关本地 `packages/host-local/src/nest/gateway/gateway.cron-route-defs.ts:22`）；`POST /v1/cron/jobs` 请求 `cronJobCreateInputSchema`（packages/contracts/src/domain/cron.ts:28）／响应 `cronJobSchema`（packages/contracts/src/domain/cron.ts:7）；`GET /v1/cron/jobs/:jobId` 请求无／响应 `cronJobSchema`（packages/contracts/src/domain/cron.ts:7）；`PATCH /v1/cron/jobs/:jobId` 请求 `cronJobUpdateInputSchema`（packages/contracts/src/domain/cron.ts:41）／响应同上；`DELETE /v1/cron/jobs/:jobId` 请求无／响应 `{ ok: true }`（`packages/host-local/src/nest/gateway/gateway.cron-route-defs.ts:106`）；`POST /v1/cron/jobs/:jobId/trigger` 请求无／响应 `cronJobSchema`（packages/contracts/src/domain/cron.ts:7）；`GET /v1/cron/status` 请求无／响应 `CronJobStatusSnapshot[]`（元素 `cronJobStatusSnapshotSchema` 见 packages/contracts/src/domain/cron.ts:63，来源 `packages/service-cron/src/deps.ts:36`）。
...
> 契约落点：`POST /v1/operations/artifacts/import` 请求 `artifactImportRequestSchema`（packages/contracts/src/domain/operations/knowledge.ts:155）／响应 `artifactImportResponseSchema`（packages/contracts/src/domain/operations/knowledge.ts:180）；`POST /v1/operations/artifacts/export` 请求 `artifactExportSchema`（网关本地 `packages/host-distributed/src/gateway/route-defs/shared.ts:265`）／响应 `artifactExportResponseSchema`（packages/contracts/src/domain/operations/governance.ts:110）；`POST /v1/operations/artifacts/activate` 请求 `artifactActivateSchema`（网关本地 `packages/host-distributed/src/gateway/route-defs/shared.ts:269`）／响应 `activationResponseSchema`（packages/contracts/src/domain/operations/governance.ts:166）；`GET /v1/operations/artifacts/review-queue` 请求无／响应 `skillReviewQueueResponseSchema`（packages/contracts/src/domain/operations/identity.ts:114）；`POST /v1/operations/artifacts/:artifactId/edit` 请求 `artifactIdParamsSchema`（网关本地 `packages/host-distributed/src/gateway/route-defs/shared.ts:277`，body 经 `bodyWithoutActor` 透传）／响应 `skillEditResponseSchema`（packages/contracts/src/domain/operations/identity.ts:13）；`GET /v1/operations/artifacts/:artifactId/history` 请求 `artifactIdParamsSchema`（网关本地 `packages/host-distributed/src/gateway/route-defs/shared.ts:277`）／响应 `skillHistoryResponseSchema`（packages/contracts/src/domain/operations/identity.ts:63）；`POST /v1/operations/artifacts/:artifactId/review` 请求 `artifactReviewSchema`（网关本地 `packages/host-distributed/src/gateway/route-defs/shared.ts:281`）／响应 `skillReviewDecisionResponseSchema`（packages/contracts/src/domain/operations/identity.ts:132）；`POST /v1/operations/artifacts/:artifactId/deactivate` 请求 `artifactIdParamsSchema`（网关本地 `packages/host-distributed/src/gateway/route-defs/shared.ts:277`，body 经 `bodyWithoutActor` 透传）／响应（未收敛，2026-09-08）。
...
> 退役面契约落点（历史参考，以 `ec0e4c99` 为准）：`GET /v1/operations/stats/usage` 请求（未收敛，2026-09-08）／响应（未收敛，2026-09-08）；`GET /v1/operations/stats/hits` 请求（未收敛，2026-09-08）／响应（未收敛，2026-09-08）；`GET /v1/operations/stats/summary` 请求（未收敛，2026-09-08）／响应（未收敛，2026-09-08）；`POST /admin/boundary-search` 请求 `adminBoundarySearchQuerySchema`（packages/contracts/src/domain/admin.ts:50）／响应 `adminBoundarySearchResponseSchema`（packages/contracts/src/domain/admin.ts:82）。
