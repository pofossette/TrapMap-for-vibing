...
| Light default host entry point (frozen mainline) | `plan.md` + `docs/todos/backend-build-targets-plan.md` + `packages/host-local/src/nest/app.module.ts` + `packages/host-local/src/nest/*/*.module.ts` (six bounded-context modules) + `packages/host-local/src/nest/main.ts` | `README.md`, `docs/README.md`, `docs/architecture/DEPLOYMENT.md`, `docs/PACKAGES.md`, `packages/host-local/package.json` (⚠ current main/dev/start still point to Fastify rollback path src/index.ts — known drift until Phase 4 cutover) |
...
| Host-local Fastify rollback path defaults | `packages/host-local/src/config/host-config.ts` + `packages/host-local/src/bootstrap/server.ts` | `README.md`, `docs/guides/MIGRATION_GUIDE.md`, `packages/host-local/README.md`, `docs/architecture/DEPLOYMENT.md` |
...
8. **`packages/server` 是供 `host-local` Fastify rollback path 使用的 Fastify compatibility shell：在迁移窗口内仅保留 legacy route compatibility 与 shared runtime/status seam，不再承担默认 light 宿主职责。** `packages/backend-core` 定义 command/use-case/port contract，`packages/host-local/src/nest/**` 是冻结后的 `light` 默认主入口终局，`packages/host-distributed` 是 `heavy` 的真实重宿主实现；maintenance batch 和 decay batch 已降级为 compatibility-only，candidate apply-resolution 与 knowledge review 是 Fastify rollback path 关闭前的最后两个 legacy 写入口。
...
15. Compatibility-shell closeout fact: `review` owns governance queue/decision/feedback/maintenance/decay commands but must delegate final aggregate mutation through `KnowledgeWritePort`; `candidate-ingestion` owns candidate workflow facts but must publish resolved results through `KnowledgeWritePort`; `packages/server` keeps legacy route compatibility only for the Fastify rollback path, and only maintenance batch 和 decay batch 当前统一返回 `501 capability_unsupported`。candidate apply-resolution 与 knowledge review 仍由该 rollback path 承接，直到 `light` 默认入口完成 Nest cutover 并且不再依赖这些 legacy 写路由。
16. Phase 4 closeout fact: the repository-level owner matrix, migration-window closure conditions, compatibility-shell retirement list, and distributed mature-service closeout task categories are frozen in `plan.md` and `docs/todos/nestjs-service-evolution-04-data-runtime-and-cutover.md`; follow-up docs must not invent a second matrix or claim a compatibility shell has already retired when the current default entrypoint still depends on it.
