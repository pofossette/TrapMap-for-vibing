...
| Nest service evolution Phase 2 modular monolith boundary freeze | `plan.md` + `docs/todos/nestjs-service-evolution-02-modular-monolith-cutover.md` + `packages/backend-core/src/<context>/{domain,application,module.ts,index.ts}` (six context directories) + `packages/backend-core/src/modules/*.ts` (compatibility re-export window) + `packages/host-local/src/nest/app.module.ts` + `packages/host-local/src/nest/*/*.module.ts` (six bounded-context Nest modules) + `packages/service-*/src/deps.ts` | `docs/README.md`, `docs/PACKAGES.md`, `docs/architecture/ARCHITECTURE.md`, `docs/reference/REPO_STRUCTURE.md`, package README |
