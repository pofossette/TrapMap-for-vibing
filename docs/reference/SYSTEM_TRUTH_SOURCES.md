...
| Server layer ownership (`domain` / `application` / `infrastructure` / `interfaces/http` / `interfaces/worker`) | `docs/plans/backend-engineering-roadmap/stage-1-foundation-and-boundaries.md` + `packages/server/README.md` + `packages/server/src/lib/README.md` + `packages/server/src/routes/README.md` | `docs/architecture/ARCHITECTURE.md`, `docs/PACKAGES.md` |
...
| Server bounded contexts and layer ownership | `docs/plans/backend-engineering-roadmap/stage-1-foundation-and-boundaries.md` | `plan.md`, `docs/architecture/ARCHITECTURE.md`, `docs/PACKAGES.md` |
...
7. **`store.snapshot()` / `store.transact()` usage is restricted to an explicit allowlist.** The guard test at `packages/server/src/__tests__/snapshot-usage-guard.test.ts` enforces this. Allowed categories:
  ...
  - **Application-service compatibility seams** (`lib/knowledge/application-service.ts`, `lib/knowledge/review-application-service.ts`, `lib/decay/application-service.ts`): explicit Stage 1 migration debt only for named capability gaps. `lib/knowledge/review-application-service.ts` no longer reads aggregate state from compatibility snapshot; its remaining `store.transact()` is a local audit seam. `lib/decay/application-service.ts` no longer reads current aggregate state from compatibility snapshot; its remaining `store.transact()` is the named `supersede` migration seam until repo-backed transaction support is in place.
...
8. Known Stage 1 read-side exceptions are tracked explicitly: `routes/review.ts` review-queue projection and `routes/decay.ts` entries/search projection still mix query filtering and response assembly, but write-heavy review/decay workflows must delegate into application services. `routes/operations/status.ts` and `routes/operations/audit.ts` no longer read compatibility snapshot directly; operator read-side snapshot access is now localized to `lib/operations/read-model.ts`.
9. All pull requests that touch architecture or persistence docs must verify consistency against this table.
