...
| Nest modular monolith entry point (opt-in migration track) | `packages/host-local/src/nest/app.module.ts` + `packages/host-local/src/nest/*/*.module.ts` (six bounded-context modules) + `packages/host-local/src/nest/main.ts` | `README.md`, `docs/README.md`, `docs/architecture/DEPLOYMENT.md`, `docs/PACKAGES.md` |
...
| Nest service evolution Phase 2 modular monolith boundary freeze | `plan.md` + `docs/todos/nestjs-service-evolution-02-modular-monolith-cutover.md` + `packages/backend-core/src/<context>/{domain,application,module.ts,index.ts}` (six context directories) + `packages/backend-core/src/modules/index.ts` (direct re-export barrel, shim files retired in Phase 4) + `packages/host-local/src/nest/app.module.ts` + `packages/host-local/src/nest/*/*.module.ts` (six bounded-context Nest modules) + `packages/service-*/src/deps.ts` | `docs/README.md`, `docs/PACKAGES.md`, `docs/architecture/ARCHITECTURE.md`, `docs/reference/REPO_STRUCTURE.md`, package README |
...
8. **`packages/server` is a partial compatibility shell for candidate/review/maintenance/decay writes.** `packages/backend-core` defines command/use-case/port contracts and `packages/host-local` / `packages/host-distributed` provide host assembly and concrete port wiring; maintenance batch 和 decay batch 已降级为 compatibility-only，candidate apply-resolution 与 knowledge review 仍是 `local-agent` / `team-monolith` 默认 Fastify 路径上的迁移窗口例外。
