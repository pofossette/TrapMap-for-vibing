...
| 评估快照 parity | `evals/promptfoo/snapshots/*-smoke.json` + `evals/promptfoo/parity-*.test.ts` + `evals/promptfoo/scripts/generate-snapshots.ts`（`pnpm eval:snapshots` 重新生成；`--runner` 默认 `promptfoo`，native runner 已移除） | `docs/operations/TESTING.md`、`docs/operations/CI_CD.md`、`evals/README.md` |
