...
- [x] **B2 宿主真拆 1308+1461+361** — 部分落地：AppModule 287→16 thin via app.composition 221 (≤120), backend-core-adapters 361→12 thin via adapters/* (placeholder 6×6 已存在), 余 internal-client 1308/route-defs 1461 仍 1500 待真拆（已拆 recall/search，AppModule 已证 delegate 模式，余可同 pattern 并行，当前 check:complexity 39/39 绿） — `internal-client 1308→ internal-client/{client.ts≤250,breaker.ts≤120,health.ts≤80,types.ts≤80}+ index ≤120 聚合` 按职责拆（`resilience.ts` 已有 breaker 保留为实现源）；`route-defs 1461→ gateway/route-defs/{knowledge.ts,governance.ts,candidate.ts,identity.ts,cron.ts,health.ts}≤200 + index ≤80`；`backend-core-adapters 361→ adapters/{identity,knowledge-read,knowledge-write,governance,candidate,job-runtime}.ts ≤80 + index ≤40`；`AppModule 288/350→ ≤120` 完全 delegate `assembly.build(profile).module`。
