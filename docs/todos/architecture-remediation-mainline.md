...
- [ ] **B1 P1 真拆 586+391** — `retrieval-recall-coordinator 587→ channel-registry 15 + hybrid 120 + semantic 120 + graph 120 + coordinator ≤150` 真实现（`@trapmap/backend-core` channelMerge port，`@trapmap/lib` tokenize/cosine 复用）；`search-knowledge 392→ search-v2 ≤180 + search-v3-plan ≤180 + 薄包装 ≤80` 真逻辑；删 `// @ts-nocheck`， budgets 由 1500 → 400/300。
- [ ] **B2 宿主真拆 1308+1461+361** — `internal-client 1308→ internal-client/{client.ts≤250,breaker.ts≤120,health.ts≤80,types.ts≤80}+ index ≤120 聚合` 按职责拆（`resilience.ts` 已有 breaker 保留为实现源）；`route-defs 1461→ gateway/route-defs/{knowledge.ts,governance.ts,candidate.ts,identity.ts,cron.ts,health.ts}≤200 + index ≤80`；`backend-core-adapters 361→ adapters/{identity,knowledge-read,knowledge-write,governance,candidate,job-runtime}.ts ≤80 + index ≤40`；`AppModule 288/350→ ≤120` 完全 delegate `assembly.build(profile).module`。
- [ ] **B3 P4 Owner-Local 真反转** — 6 service `schema.ts` 自定义 `pgTable`（`knowledge 561/800, artifacts 449, candidates 154, auth 172, queue 55, retrieval 79, experience-genes 145, labels 162, cron 31, column-factories 151` 拆至 Owner），`packages/db/src/schema/index.ts` 仅 `export * from '@trapmap/service-*/schema'` 聚合；更新 `check-table-schema` 与 `check-pgtable-single-source` 扫描 Owner 源；`conflict_relations` 归 `service-governance-review/schema.ts`。
- [ ] **B4 P5 缓存真接线** — `CachePort`（`@trapmap/backend-core/ports/cache-port`） 接 `service-knowledge-read/recall/*,store/pg.go`（`get→cache hit→return; miss→pgRecall→set`，key=`sha256(canonicalJson)` via `@trapmap/lib`），`service-job-runtime/cache-invalidation.ts` 经 `workflow_runs/outbox` 失效；`services/knowledge-read-go/internal/cache` 复用 `lru+singleflight`；指标 `hitRate` p95>60% 可测。
- [ ] **B5 P6 契约真拆** — `operations 659/700→ identity/knowledge/governance/job 4×≤180 + index 10` 已落地，补 `test 3424→ governance,knowledge,identity,job 4 测 + index` 按域拆；`generate:contracts --check` + `check:go-contract` 进 CI；深路径 lint 阻止 `from '.../operations'` 直引。
- [ ] **B6 Fallow 清零** — `recall/search/*` placeholder 导致 30 dead /10 complexity /8 dup 清零或加 `.fallowrc` allowlist；移除无用 `export *` 与 `// @ts-nocheck`；`pnpm exec fallow audit --base main` 0。
- [ ] **B7 全量门禁** — `pnpm check:complexity 37/37→ budgets 全 ≤400/300 无 1500`, `check:docs 7/7`, `check:table-schema 42/42`, `check:structure 3/3`, `typecheck 0`, `mermaid 116`, `go vet/test 6 pkgs`, `gov 38+ kw 34 + read 123` 绿；落 `docs/archived/archived-plans/architecture-remediation-b-true-convergence.md`。
