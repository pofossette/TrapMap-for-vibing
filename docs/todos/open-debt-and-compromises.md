...
- [ ] **设计要点（v2，按用户澄清修订：只考虑 TS，不引入 json/yml 装配）：** 新包 `packages/assembly`（引入 `@deepseek-ai/cordis` 4.x 编程式装配，不自研 DI 图、不引入 loader/patch 配置文件）；7 个 service 包各加薄包装文件 `plugin.ts`（现有 create<X>Deps/Module/RouteDefs 原样复用，业务零改动）；host-* 收敛为 transport 插件（Nest/Fastify adapter 不动）；部署形态 = TS 组合器（localAgentAssembly / teamMonolithAssembly / distributedAssembly 三个普通 TS builder，顺序 = 有序 bundles 语义，无任何 yml/json）；四阶段迁移（地基→host-local 试点→host-distributed 收敛→双实现收敛收尾）。
