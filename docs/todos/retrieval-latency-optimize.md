...
| P1-1 | 本提交 | 胶囊全文表达式 GIN（与查询表达式逐字对齐） | 1000: 9.78 / **9.90** / **27.50** / 11.42 / 1.25 | v2 keyword 26→12（ts_rank 需逐行计算，GIN 只加速匹配集，符合预期）；数据 `optimize/after-p1-1000.json` |
| P1-2 | 本提交 | 胶囊池 TTL 缓存（snapshot 23→0.5）+ v1-skills 走读模型缓存（弃 `listForRetrieval({})` 直连） | 同上行：v2 snapshot 归零；**v1-skills 87.85→9.90（8.9×）** | skills 与 v1-search 拉平；实现要点：loadArtifacts 返回类型用读模型元素类型，规避 exactOptionalPropertyTypes 的 `\| undefined` 不可赋值问题 |
