...
- 2026-07-07 23:24-23:25 CST 已用本地 Docker Compose 启动的官方 Langfuse v3 实例完成 live closeout：`LANGFUSE_BASE_URL=http://127.0.0.1:3000`，同轮 `pnpm --filter @trapmap/evals eval -- smoke --platform langfuse` 输出三条 success evidence，且 native TrapMap smoke 仍以 `81/81 passed` 结束。
