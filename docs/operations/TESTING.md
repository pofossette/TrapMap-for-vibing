...
| 表清单守卫 | `pnpm check:table-schema` | `persistence-schema` 的 65 张 `pgTable` vs `DATABASE_SCHEMA.md` 表清单 diff（缺表/幽灵表/分节计数不一致即失败） |
...
表清单守卫以 `packages/persistence-schema/src/` 的 65 张 `pgTable` 为权威；`store_snapshot` 属迁移 SQL 历史残留，不在守卫范围内。CI 中由 `doc-guardrails` job 自动执行。
