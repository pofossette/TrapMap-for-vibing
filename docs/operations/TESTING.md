...
> **说明**：本文档中大量测试命令引用 `归档旧实现` 路径，该包已于  删除。当前测试命令请使用 `pnpm test:file -- <repo-root-relative-test-path>` 格式，路径指向当前存在的包（如 `packages/service-*`、`packages/host-local`、`packages/contracts` 等）。历史路径详见 `docs/archived/archived-plans/compatibility-shell-retirement-runtime-infra-ownership.md`。
...
这些根脚本现在通过 `scripts/run-dev.ts` 经 backend target registry 分发到 `@trapmap/app-light` 与 `@trapmap/app-distributed`（组装中心；宿主库包 `@trapmap/host-local` / `@trapmap/host-distributed` 由 registry 的 `libraryPackage` 字段保留）。兼容别名 `pnpm dev:local-agent`、`pnpm dev:team-monolith`、`pnpm dev:distributed:*` 仍可用。测试命令里仍然大量引用 `归档旧实现/...`，是因为当前权威测试文件与核心实现仍主要驻留在该兼容层和既有代码面中。

**Phase 2 Store Snapshot / PG-first Freeze Checks:**
- Snapshot allowlist：运行 `归档旧实现/src/__tests__/snapshot-usage-guard.test.ts`，确认新的 `store.snapshot()` / `store.transact()` 调用没有逃出 allowlist，并且 allowlist 仍只覆盖命名 compatibility buckets。
- PG-first compatibility：运行 `归档旧实现/src/__tests__/pg-first-compat.test.ts`，确认 access-key / member 等 PG-first surface 在 InMemory fallback 下仍维持相同外部 contract；这证明 InMemory 当前是 compatibility/testing posture，而不是第二套 owner 语义。
- Truth freeze：运行 `pnpm check:docs`，确认 remediation detail plan、truth source、packages doc、persistence doc 与 testing doc 对 `store_snapshot` / InMemory / PG-first 口径的描述一致。
...
- Phase 3 runtime metrics export：运行 `归档旧实现/src/app.test.ts`，确认 `/metrics` 返回 Prometheus text，包含 `trapmap_runtime_executions_total`、`trapmap_runtime_request_duration_ms_*` 等真实样本，并且 label 中不出现 `requestId` / `traceId` 这类高基数键。
...
- Phase 3 structured logging：运行 `归档旧实现/src/app.test.ts` 与 `归档旧实现/src/lib/runtime/resilience.test.ts`，确认 request / resilience 日志至少带 `eventCategory`、`eventName`、`requestId`、`traceId`、`serviceName`、必要时 `attempt`。
...
- Script export flow：运行 `pnpm exec tsx scripts/archived/export-badcase-to-eval.ts <feedbackId> <outputPath>`，确认输出文件与 route `draft` shape 一致，并且输出只包含 `badcaseEvalDraftSchema`；route 额外携带的 `debug` 仅用于 operator/debug 闭环，不属于 eval draft payload。
...
- Phase 1：至少运行 `归档旧实现/src/app.test.ts`、`归档旧实现/src/bootstrap/startup.test.ts`、`归档旧实现/src/config.test.ts`，并确认 `ARCHITECTURE.md`、`SYSTEM_TRUTH_SOURCES.md` 与相关 README 已回写 ownership / allowlist。
- Phase 2：至少运行 `归档旧实现/src/routes/operations/status.test.ts` 与 async/runtime 相关测试，确认 `/v1/operations/status/async` contract、`workflow_runs.stats` checkpoint source 和 failure taxonomy 已冻结。
...
- Phase 3：至少运行 `归档旧实现/src/routes/operations/status.test.ts`、`归档旧实现/src/routes/operations/stats.test.ts`、`归档旧实现/src/config.test.ts`，确认 operatorHome / configGovernance / capacityModel / bulkOperations 以及 cache invalidation summary 已落地。
...
| `cron` | `service-cron` | cron job CRUD/trigger/status + scheduler 认领 | 只拥有 `cron_jobs` 注册表 |
...
2. `pnpm test:file -- 归档旧实现/src/routes/operations/badcases.test.ts`
...
pnpm exec vitest run \
  归档旧实现/src/lib/queue/task-queue.test.ts \
  归档旧实现/src/lib/candidates/processor.test.ts \
  归档旧实现/src/__tests__/candidate-pipeline.test.ts

pnpm exec vitest run \
  packages/contracts/src/domain/candidates.test.ts \
  归档旧实现/src/lib/candidates/detector.test.ts \
  归档旧实现/src/lib/candidates/pg-detector.test.ts \
  归档旧实现/src/lib/candidates/pg-repository.test.ts \
...
# Runtime surface
pnpm test -- --run \
  归档旧实现/src/app.test.ts \
  归档旧实现/src/lib/runtime/runtime-metadata.test.ts \
  归档旧实现/src/config.test.ts

# Shared resilience primitives
pnpm test -- --run \
  归档旧实现/src/lib/runtime/resilience.test.ts \
  归档旧实现/src/lib/runtime/metrics.test.ts \
  归档旧实现/src/lib/candidates/processor.test.ts \
  归档旧实现/src/bootstrap/startup.test.ts \
  归档旧实现/src/lib/indexing/graph-lite/llm-extract.test.ts

# Async reliability
pnpm test -- --run \
  归档旧实现/src/lib/queue/task-queue.test.ts \
  归档旧实现/src/lib/lifecycle/outbox.test.ts \
  归档旧实现/src/__tests__/candidate-pipeline.test.ts \
  归档旧实现/src/lib/lifecycle/subscribers/subscribers-integration.test.ts \
  归档旧实现/src/routes/candidates.test.ts
...
| 变更类型 | 必须运行的验证 |
|----------|--------------|
| 文档修改 | `pnpm check:docs` + `pnpm check:deps` |
...
| `Phase 1` | `pnpm test -- --run 归档旧实现/src/app.test.ts 归档旧实现/src/bootstrap/startup.test.ts 归档旧实现/src/config.test.ts` + `pnpm check:docs` + `pnpm check:structure` |
| `Phase 2` | `pnpm test -- --run 归档旧实现/src/routes/operations/status.test.ts 归档旧实现/src/lib/runtime/runtime-metadata.test.ts 归档旧实现/src/config.test.ts` + `pnpm check:docs` + `pnpm check:structure` |
...
# 意图解析器测试（正则 + LLM）
pnpm test -- --run 归档旧实现/src/lib/retrieval/capsules/intent.test.ts

# 意图缓存测试
pnpm test -- --run 归档旧实现/src/lib/retrieval/capsules/intent-cache.test.ts

# 运行特定文件
pnpm vitest run evals/retrieval/runner.test.ts

# 从仓库根可靠运行单个测试文件
pnpm test:file -- 归档旧实现/src/lib/runtime/metrics.test.ts
```
...
```bash
pnpm test -- --run \
  归档旧实现/src/routes/feedback.test.ts \
  归档旧实现/src/routes/retrieval.test.ts \
  归档旧实现/src/routes/review.test.ts \
  归档旧实现/src/routes/operations/skill-edit.test.ts \
  归档旧实现/src/routes/operations/skill-review.test.ts
```
...
```bash
pnpm test -- --run 归档旧实现/src/routes/access-keys.test.ts 归档旧实现/src/routes/auth.test.ts 归档旧实现/src/routes/members.test.ts 归档旧实现/src/__tests__/pg-first-compat.test.ts
```
...
守卫测试位于 `归档旧实现/src/__tests__/snapshot-usage-guard.test.ts`，扫描所有非测试 `.ts` 文件。新增不允许列表中的 `store.snapshot()` / `store.transact()` 调用会导致测试失败。
...
fm-agent 针对 `归档旧实现` 生成了 391 个已确认的原始发现（原始快照）。当前 HEAD 已显著领先原始快照（buildServer、capsule-native 检索等均已落地）。2026-05-29 审计回写后，matrix 中已无 current-live finding；完整分类矩阵见 `docs/plans/fm-agent-scan/server-live-gap-matrix.md`。
...
| 测试文件 | 覆盖的活跃问题 |
|---|---|
| `归档旧实现/src/app.test.ts` | 已修复：onClose await worker stop、startup 后冻结 `skillShareer` |
| `归档旧实现/src/bootstrap/startup.test.ts` | 已修复 / 已文档化边界：生命周期审计订阅补齐；JSON store recovery 不重入 PG queue |
| `归档旧实现/src/lib/ai/dynamic/context-resolver.test.ts` | 已文档化边界：MCP 状态当前显式为 `unavailable` |
| `归档旧实现/src/lib/ai/provider-config.test.ts` | 已修复：provider-specific key 优先级 |
...
# 仅运行 fm-agent 回归冻结测试
pnpm --filter @trapmap/server test -- \
  --run 归档旧实现/src/app.test.ts \
  归档旧实现/src/bootstrap/startup.test.ts \
  归档旧实现/src/lib/ai/dynamic/context-resolver.test.ts \
  归档旧实现/src/lib/ai/provider-config.test.ts
...
`store_snapshot` 的一次性 source-to-target backfill 已在 2026-07-23 以独立 PostgreSQL source 与已迁移的 six-owner target 完成验收。提前加入 identity-access baseline 的 destructive migration 会破坏仍在运行的 compatibility server，已撤回；只有  删除全部 runtime consumer 后，才可在同一 cutover 中重新引入该 migration。用于该演练的命令、wiring 与 acceptance test 已删除，仓库不再提供可执行的 legacy snapshot backfill 命令。
...
### backfill:labels（一次性运维脚本，已归档）

从历史数据（`knowledge_labels`、artifact `labels`、`graph_index_documents`）中回填规范标签目录。脚本已归档至 `scripts/archived/`，不再提供 pnpm 便捷入口。

```bash
# 运行回填
pnpm exec tsx --tsconfig tsconfig.base.json scripts/archived/backfill-labels.ts

# 预览模式（不写入数据库）
pnpm exec tsx --tsconfig tsconfig.base.json scripts/archived/backfill-labels.ts --dry-run
```

要求：`DATABASE_URL` 环境变量已设置。

### label-merge:repair（一次性运维脚本，已归档）

标签合并后修复图文档中的节点 ID 和边端点。脚本已归档至 `scripts/archived/`，不再提供 pnpm 便捷入口。

```bash
# 运行修复
pnpm exec tsx --tsconfig tsconfig.base.json scripts/archived/repair-label-merges.ts

# 预览模式（不写入数据库）
pnpm exec tsx --tsconfig tsconfig.base.json scripts/archived/repair-label-merges.ts --dry-run
```
