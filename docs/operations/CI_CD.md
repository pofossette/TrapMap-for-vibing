...
| `doc-guardrails` | `pnpm check:docs` + `pnpm check:structure` + `pnpm check:asserts` + `pnpm check:table-schema` + `pnpm check:pgtable-single-source` + `pnpm check:eval-imports` + `pnpm check:eval-only` + `pnpm check:deps` + `pnpm check:complexity` | 文档守卫（doc-drift / mermaid / md-lint 阻断层 + doc-truth / doc-references / links 可见层）、结构守卫（structure / arch-freeze / stale-package-refs）、裸断言守卫、表清单守卫、pgTable 单源守卫、eval import 边界守卫、@eval-only 标记守卫、依赖分析、复杂度预算 |
...
`doc-guardrails` job 运行合并后的文档守卫（`pnpm check:docs`）、结构守卫（`pnpm check:structure`）、裸断言守卫（`pnpm check:asserts`）、表清单守卫（`pnpm check:table-schema`）、pgTable 单源守卫（`pnpm check:pgtable-single-source`）、eval import 边界守卫（`pnpm check:eval-imports`）、@eval-only 标记守卫（`pnpm check:eval-only`）、依赖分析守卫（`pnpm check:deps`）和复杂度预算守卫（`pnpm check:complexity`），确保关键文档不含过时内容、仓库结构完整、热点文件未超出行数预算、无新增裸类型断言、`DATABASE_SCHEMA.md` 表清单与 `persistence-schema` 对齐、service 包不重定义表、evals 不直连 service 内部文件且 eval-only 模块带标记、依赖关系无循环。`check:docs` 内部保留独立失败定位：doc-drift / mermaid / md-lint 是阻断层，doc-truth / doc-references / links 是可见但非阻断层（保持历史 `|| true` 语义）；`check:structure` 内部包含 structure / arch-freeze / stale-package-refs 三个子检查。表清单守卫以 `packages/persistence-schema/src/` 的 64 张 `pgTable` 为权威，diff 出 `DATABASE_SCHEMA.md` 的缺表/幽灵表/分节计数不一致即失败；`store_snapshot` 属于迁移 SQL 历史残留，不在守卫范围内。pgTable 单源守卫要求 `packages/service-*` 的 schema.ts 只 re-export `@trapmap/persistence-schema`。eval import 边界守卫允许 evals 经 `@trapmap/*` 包名、`packages/contracts/**`、host-local eval allowlist 与 `@eval-only` 标记模块接入，其余深路径直连即失败。@eval-only 守卫要求仅被 evals 引用且无产品消费者的模块带 `@eval-only` 头注释。架构边界与未使用依赖守卫由 `fallow-push-audit` job 的 `pnpm check:fallow` 统一覆盖，不再在 `doc-guardrails` 中重复调用。漂移规则覆盖以下类别：
