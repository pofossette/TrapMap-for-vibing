 ...
 * create<X>Server sequence moved into the distributed assembly layers
 * (config/database nodes + the knowledge-read server node, which owns the
 * read-model projection + retrieval query); this starter is now a thin caller
 * of the shared profile boot.
 */
