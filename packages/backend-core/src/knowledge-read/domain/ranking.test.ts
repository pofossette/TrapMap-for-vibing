...
function createEntry(id: string, overrides: Partial<Entry> = {}): Entry {
  return { id, scope: 'global', labels: [], shortcut: '', detail: '', decayMeta: null, ...overrides };
}
...
describe('knowledge-read ranking rules', () => {
  it('merges semantic and keyword candidates with fixed weights', () => {
    ...
    const keyword: RecallCandidateLike<Entry>[] = [
      { entry: createEntry('a'), channel: RECALL_CHANNEL_KEYWORD, score: 0.4, tokenMatches: [{ token: 'x', fields: ['shortcut'] }] },
  ...
  it('infers routing channels from merged candidates', () => {
    ...
    expect(
      inferChannelsFromMerged([{ channels: [RECALL_CHANNEL_KEYWORD] }, { channels: [RECALL_CHANNEL_GRAPH] }]),
  ...
  it('maps explicit query modes to planned channels', () => {
    ...
    expect(routingDecision('hybrid').channelsPlanned).toEqual([RECALL_CHANNEL_SEMANTIC, RECALL_CHANNEL_KEYWORD]);
    expect(routingDecision('graph-assisted').channelsPlanned).toEqual([RECALL_CHANNEL_SEMANTIC, RECALL_CHANNEL_KEYWORD, RECALL_CHANNEL_GRAPH]);
  ...
  it('computes semantic scores with label, scope and lexical boosts', () => {
    const entry = createEntry('e', { labels: ['deploy'], scope: 'project', shortcut: 'deploy k8s', detail: 'deploy k8s cluster' });
