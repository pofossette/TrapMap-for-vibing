...
const exactVersionMatch: VersionRangeMatcher = (_actual, _expected, comparison) =>
  comparison === 0;
    ...
    (entry) =>
      entry.boundary?.versions.every((constraint) => {
        const version = context.versions?.find(
          (item) =>
            item.package.toLowerCase().trim() === constraint.package.toLowerCase().trim(),
        );
...
export function contextScoreDelta(
  entry: BoundaryScoredEntryView,
  queryContext: string,
): number {
