...
describe('knowledge-read tokenization rules', () => {
  ...
  it('scores keyword candidates by label, shortcut and detail weights', () => {
    ...
    const expectedScore = (KEYWORD_LABEL_WEIGHT + KEYWORD_DETAIL_WEIGHT + KEYWORD_SHORTCUT_WEIGHT + KEYWORD_DETAIL_WEIGHT) / (maxFieldScore * tokens.length);
  ...
  it('returns a zero score for an empty query', () => {
    expect(scoreKeywordEntry([], { labels: new Set(), shortcut: new Set(), detail: new Set() })).toEqual({ score: 0, tokenMatches: [] });
  });
  ...
  it('clamps keyword scores into the 0..1 range', () => {
    ...
    const { score } = scoreKeywordEntry(
      ['react'],
      { labels: new Set(['react']), shortcut: new Set(['react']), detail: new Set(['react']) },
    );
    expect(score).toBe(1);
  });
});
