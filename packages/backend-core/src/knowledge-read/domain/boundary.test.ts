...
describe('knowledge-read boundary rules', () => {
  ...
  it('keeps entries without boundary metadata', () => {
    expect(filterByBoundary([{ boundary: null }], { versions: [{ package: 'react', version: '3.0.0' }] })).toHaveLength(1);
    expect(filterByBoundary([{}], { versions: [{ package: 'react', version: '3.0.0' }] })).toHaveLength(1);
  });
