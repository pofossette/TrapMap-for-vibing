...
function createEntry(overrides: Partial<RetrievalEligibilityEntryView> = {}): RetrievalEligibilityEntryView {
  return {
    lifecycleState: 'approved',
    decayMeta: { lastVerifiedAt: '2026-01-01T00:00:00.000Z', decayState: 'active', supersededById: null },
    teamId: null,
    requiredLevel: 0,
    scope: 'global',
    labels: [],
...
describe('knowledge-read eligibility rules', () => {
  it('derives decay state from last verification age', () => {
    ...
    expect(decayStateForAge(new Date(now - 120 * 86_400_000).toISOString(), config)).toBe('review-due');
    expect(decayStateForAge(new Date(now - 200 * 86_400_000).toISOString(), config)).toBe('stale');
    expect(decayStateForAge(new Date(now - 400 * 86_400_000).toISOString(), config)).toBe('expired');
  });

  it('preserves the persisted decay state when decay is disabled', () => {
    expect(computeDecayState({ lastVerifiedAt: '2020-01-01T00:00:00.000Z', decayState: 'active', supersededById: null }, { ...DEFAULT_DECAY_CONFIG, enabled: false })).toBe('active');
  });

  it('lets superseded entries win over age-based decay', () => {
    expect(computeDecayState({ lastVerifiedAt: '2026-01-01T00:00:00.000Z', decayState: 'active', supersededById: 'entry-2' }, DEFAULT_DECAY_CONFIG)).toBe('superseded');
    expect(computeDecayState({ lastVerifiedAt: '2026-01-01T00:00:00.000Z', decayState: 'superseded', supersededById: null }, DEFAULT_DECAY_CONFIG)).toBe('superseded');
  });
  ...
  it('matches retrieval filters on scopes and all labels', () => {
    ...
    expect(matchesRetrievalFilters(entry, { scopes: [], labels: ['deploy', 'missing'] })).toBe(false);
  });
  ...
  it('requires the approved lifecycle state for full eligibility', () => {
    ...
    expect(isRetrievalEntryEligible(createEntry({ lifecycleState: 'submitted' }), auth, filters, DEFAULT_DECAY_CONFIG)).toBe(false);
    expect(isRetrievalEntryEligible(createEntry({ decayMeta: { lastVerifiedAt: '2020-01-01T00:00:00.000Z', decayState: 'active', supersededById: null } }), auth, filters, DEFAULT_DECAY_CONFIG)).toBe(false);
  });
});
