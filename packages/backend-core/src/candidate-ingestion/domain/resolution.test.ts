...
import { normalizeManualResolution } from './resolution.js';
...
describe('candidate-ingestion resolution rules', () => {
  ...
  it('rejects unknown decisions and empty notes', () => {
    expect(() =>
      normalizeManualResolution({ decision: 'merge', notes: 'publish it' }),
    ).toThrow('Candidate resolution requires decision "independent" or "merged"');
    expect(() =>
      normalizeManualResolution({ decision: 'merged', notes: '   ' }),
    ).toThrow('Candidate resolution requires non-empty notes');
  });

  it('requires a merged target for merged resolutions', () => {
    expect(() =>
      normalizeManualResolution({ decision: 'merged', notes: 'merge it' }),
    ).toThrow('Merged candidate resolution requires mergedWith.entityType and mergedWith.entityId');
    expect(() =>
      normalizeManualResolution({ decision: 'merged', notes: 'merge it', mergedWith: { entityType: 'skill' } }),
    ).toThrow('Merged candidate resolution requires mergedWith.entityType and mergedWith.entityId');
  });
