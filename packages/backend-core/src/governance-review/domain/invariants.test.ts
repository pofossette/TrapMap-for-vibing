...
describe('governance-review feedback invariants', () => {
  it('matches feedback to a remediation reactivation payload', () => {
    ...
    expect(feedbackMatchesRemediationReactivation(record, { entryId: 'entry-1', entryType: 'trap' })).toBe(true);
    expect(feedbackMatchesRemediationReactivation(record, { entryId: 'entry-9', entryType: 'trap' })).toBe(false);
    expect(feedbackMatchesRemediationReactivation(record, { entryId: 'entry-1', entryType: 'skill' })).toBe(false);
  });
  ...
  it('treats null and absent query ids as equivalent', () => {
    ...
    expect(feedbackMatchesBadcaseExport(record, { entryId: 'entry-1', entryType: 'skill', queryId: null })).toBe(true);
    const absent = { entryId: 'entry-1', entryType: 'skill' };
    expect(feedbackMatchesBadcaseExport(absent, { entryId: 'entry-1', entryType: 'skill', queryId: null })).toBe(true);
  });
});
