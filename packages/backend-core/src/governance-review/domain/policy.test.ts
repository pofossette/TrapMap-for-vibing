...
describe('governance-review feedback policy', () => {
  ...
  it('filters active feedback per entry', () => {
    ...
    expect(activeFeedback(records, 'entry-1').map((record) => record.id)).toEqual([
      'fb-1',
      'fb-2',
    ]);
  });
  ...
  it('derives pending-human-review state with suppressed flags and ids', () => {
    const records = Array.from({ length: 10 }, (_, index) =>
      feedback({ id: `fb-${index + 1}`, submittedAt: `2026-07-${String(index + 1).padStart(2, '0')}T00:00:00.000Z` }),
    );
...
describe('governance-review lifecycle trigger rules', () => {
  const rule = { problemType: 'outdated', minCount: 3, timeWindowDays: 30, targetDecayState: 'stale' as const };
