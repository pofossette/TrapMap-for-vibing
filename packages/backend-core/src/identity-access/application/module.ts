/**
 * Identity-access bounded context — application layer.
 *
 ...
 *
 * Host assemblies provide concrete repository and lookup ports; the
 * factory here is the only place that knows how to compose them into the
 ...
 * Consumed by the service topology to determine which service unit
 * owns these capabilities.
 */
export const IDENTITY_ACCESS_MODULE = {
  name: 'identity-access' as const,
  owns: IDENTITY_ACCESS_OWNED_CAPABILITIES,
  dependsOn: [] as const,
} as const;
...
export function createIdentityAccessModule(deps: IdentityAccessDeps): IdentityAccessPort {
  return {
    ...
    async loginSystemAdmin(systemAdminKey: string) {
      ...
      const sessionToken = composeSystemAdminSessionToken(randomBytes(32).toString('base64url'));
