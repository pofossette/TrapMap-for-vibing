...
export function finalizeSemanticResults(
...
export function versionMultiplierFor(
  infra: NonNullable<ReturnType<typeof getRetrievalInfra>>,
...
export async function rerankRecallResults(
  _infra: any,
  ...
  if ((goClient as any).isEnabled) {
    try {
      const payload = mergedCandidates.map((c: any) => ({
        id: c.entry.id,
        ...
        tokenMatches: c.tokenMatches.map((tm: any) => ({
          token: tm.token,
          fields: [...tm.fields],
        })),
        channels: [...c.channels],
        ...
        decayState: c.entry.decayMeta?.freshnessType,
        boundary: c.entry.boundary,
      }));
      const result = await (goClient as any).rankingBatch({
        entries: payload,
        queryTokens,
        maxCandidates: parsed.maxResults,
      });
      const entryMap = new Map(mergedCandidates.map((c: any) => [c.entry.id, c.entry]));
      const scoredEntries: ScoredEntry[] = result.candidates
      ...
      return {
        scoredEntries,
        mergedCandidates: result.candidates
          .map((r: any) => entryMap.get(r.id)!)
  ...
  const scored = mergedCandidates
    .map((c: any) => ({ entry: c.entry, score: c.finalScore }))
    .sort((a: any, b: any) => b.score - a.score)
    .slice(0, parsed.maxResults);
  return {
    scoredEntries: scored.map(({ entry, score }: any) => toScoredEntry(entry, score)),
    mergedCandidates,
  };
}
export async function computeSemanticCandidates(
  ...
  const candidates = scoredEntries.map(({ entry, score }: any) =>
