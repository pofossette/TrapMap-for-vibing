...
import { assertKnowledgeReadMigrationSet, runKnowledgeReadMigrations } from '../src/migrations.js';

it('assertKnowledgeReadMigrationSet is deprecated no-op', async () => {
  await expect(assertKnowledgeReadMigrationSet()).resolves.toBeUndefined();
  await expect(assertKnowledgeReadMigrationSet('/tmp' as any)).resolves.toBeUndefined();
});

it('runKnowledgeReadMigrations delegates to @trapmap/db runMigrations', async () => {
  ...
  await expect(runKnowledgeReadMigrations(pool)).resolves.toBeUndefined();
