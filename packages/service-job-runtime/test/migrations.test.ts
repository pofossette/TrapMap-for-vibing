...
import { assertJobRuntimeMigrationSet, runJobRuntimeMigrations } from '../src/migrations.js';

it('assertJobRuntimeMigrationSet is deprecated no-op', async () => {
  await expect(assertJobRuntimeMigrationSet()).resolves.toBeUndefined();
  await expect(assertJobRuntimeMigrationSet('/tmp' as any)).resolves.toBeUndefined();
});

it('runJobRuntimeMigrations delegates to @trapmap/db runMigrations', async () => {
  ...
  await expect(runJobRuntimeMigrations(pool)).resolves.toBeUndefined();
