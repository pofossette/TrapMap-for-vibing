...
export function ErrorPanel({ message, onRetry, title }: ErrorPanelProps): ReactElement {
  const { t } = useI18nStore();
  const finalTitle = title ?? t('systemError');
    ...
    <div className="rounded-panel border border-rose-500/30 bg-rose-500/10 p-5 shadow-panel">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 space-y-1">
          <h3 className="text-sm font-semibold text-rose-200">{finalTitle}</h3>
