...
it('assertGovernanceReviewMigrationSet is deprecated no-op', async () => {
  await expect(assertGovernanceReviewMigrationSet()).resolves.toBeUndefined();
  await expect(assertGovernanceReviewMigrationSet('/tmp' as any)).resolves.toBeUndefined();
});

it('runGovernanceReviewMigrations delegates to @trapmap/db runMigrations', async () => {
  ...
  await expect(runGovernanceReviewMigrations(pool)).resolves.toBeUndefined();
