...
import { assertCronMigrationSet, runCronMigrations } from '../src/migrations.js';

it('assertCronMigrationSet is deprecated no-op', async () => {
  await expect(assertCronMigrationSet()).resolves.toBeUndefined();
  await expect(assertCronMigrationSet('/tmp' as any)).resolves.toBeUndefined();
});

it('runCronMigrations delegates to @trapmap/db runMigrations', async () => {
  ...
  await expect(runCronMigrations(pool)).resolves.toBeUndefined();
