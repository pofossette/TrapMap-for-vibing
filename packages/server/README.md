...
`packages/server` 是 TrapMap 的 Fastify 兼容壳与共享 runtime/status seam。默认 `light` 宿主已经切到 `packages/host-local/src/nest/**`，本包不再承担本地宿主职责，也不是共享业务内核。

## 入口

- `src/index.ts` — Fastify 兼容壳启动入口
...
### 启动序列（bootstrap）
3. `src/bootstrap/run-startup-sequence.ts` — 启动序列编排器（OTel、仓库、候选恢复、图协调、worker、生命周期共 6 个步骤）
...
> ⚠ 以上五层在 `packages/server` 内的存在是兼容收尾期的历史事实，不是长期业务归属。正式终局归属是 `packages/backend-core` + `packages/service-*`（共享实现面）+ `packages/host-local` / `packages/host-distributed`（宿主特定实现面）。

## 重点上下文落点

下面四个上下文优先按同一模型收口：

| 上下文 | 领域/应用归属 | 基础设施 / 接口归属 |
|---|---|---|
...
fm-agent 原始报告（391 个已确认发现）经过当前 HEAD 审计回写后，以下模块保留为回归入口或环境边界说明，不再有已复现的 current-live gap：
