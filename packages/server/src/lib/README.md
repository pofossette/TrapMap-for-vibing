...
| `application` | `lib/<context>/application-*.ts`、具名服务或处理器 | 面向命令用例的编排：actor 解析、聚合读取、repo 写入、生命周期 / 事件触发，以及已文档化的兼容接缝 |
...
- `domain` 和 `application` 不拥有启动编排或进程生命周期关注点。
- 运行时 readiness、worker 启动、迁移执行和恢复逻辑留在基础设施模块。
- 读模型组装应保留在读侧模块，例如 `retrieval/` 或其他投影模块；写侧应用服务应返回写入结果，而不是悄悄组装检索或运行时投影，除非该耦合已经被显式文档化。
...
| `artifacts/` | artifact 生命周期领域、仓库实现与派生制品支撑 |
...
| 上下文 | 保留在 `domain` / `application` 的内容 | 保留在 `infrastructure` / 读侧的内容 |
|---|---|---|
...
fm-agent 原始报告中的热点，与当前 `lib/` 模块的映射关系如下：
