...
function buildDeployClusterDataset() {
  const trapNodes: GraphNodeRecord[] = [
    ...
    'service-unavailable',
    'stale-cache',
  ].map((id) => ({ id: `trap:${id}`, kind: 'trap', label: id, evidence: 'test' }));
  const skillNodes: GraphNodeRecord[] = [
    ...
    'rotate-secret',
    'verify-config',
  ].map((id) => ({ id: `skill:${id}`, kind: 'skill', label: id, evidence: 'test' }));
  const cueNodes: GraphNodeRecord[] = ['timeout', 'rollback', 'kubernetes', 'deployment'].map(
    (id) => ({ id: `cue:${id}`, kind: 'cue', label: id, evidence: 'test' }),
  );
  const prerequisiteNodes: GraphNodeRecord[] = ['cluster', 'registry', 'approval'].map((id) => ({
    id: `prereq:${id}`,
    kind: 'prerequisite',
    label: id,
    evidence: 'test',
  ...
  const edges: GraphEdgeRecord[] = [
    {
      id: 'rolling-mitigates-memory',
      sourceNodeId: 'skill:k8s-rolling-update',
      targetNodeId: 'trap:mem-leak-rollback',
    ...
    },
    {
      id: 'rolling-health',
      sourceNodeId: 'skill:k8s-rolling-update',
      targetNodeId: 'skill:container-health-check',
      relationType: 'co-occurs-with',
      strength: 'soft',
      evidence: 'test',
    },
    {
      id: 'blue-green-requires-rolling',
      sourceNodeId: 'skill:blue-green-deploy',
      targetNodeId: 'skill:k8s-rolling-update',
      relationType: 'requires',
      strength: 'hard',
      evidence: 'test',
    },
    {
      id: 'canary-requires-rolling',
      sourceNodeId: 'skill:canary-release',
      targetNodeId: 'skill:k8s-rolling-update',
      relationType: 'requires',
      strength: 'hard',
      evidence: 'test',
    ...
    {
      id: 'deploy-risk-blocks-timeout',
      sourceNodeId: 'trap:deploy-timeout',
      targetNodeId: 'cue:timeout',
      relationType: 'risk-blocks',
      strength: 'hard',
      evidence: 'test',
    },
    {
      id: 'config-order-secret',
      sourceNodeId: 'skill:verify-config',
      targetNodeId: 'skill:rotate-secret',
      relationType: 'order',
      strength: 'soft',
      evidence: 'test',
    },
  ...
  return {
    graphDocs: [
      makeDoc('deploy-traps', 'trap', 'deploy-traps', 1, [...trapNodes, ...cueNodes], edges),
      makeDoc(
        'deploy-skills',
        'skill',
        'deploy-skills',
        1,
        [...skillNodes, ...prerequisiteNodes],
        [],
      ),
    ],
    trapNodes,
...
function buildDisconnectedDataset() {
  const isolatedNodeIds = ['cue:isolated-1', 'cue:isolated-2'];
  const clusterANodeIds = ['trap:cluster-a-trap', 'skill:cluster-a-skill'];
  const clusterBNodeIds = ['trap:cluster-b-trap', 'skill:cluster-b-skill'];
  return {
    docs: [
      makeDoc(
        'cluster-a-trap',
        ...
        1,
        [
          { id: 'trap:cluster-a-trap', kind: 'trap', label: 'Cluster A Trap', evidence: 'test' },
          {
            id: 'skill:cluster-a-skill',
            kind: 'skill',
            label: 'Cluster A Skill',
            evidence: 'test',
          },
          ...isolatedNodeIds.map((id) => ({
            id,
            kind: 'cue' as const,
            label: id,
            evidence: 'test',
        ...
        [
          {
            id: 'cluster-a-edge',
            sourceNodeId: 'trap:cluster-a-trap',
            targetNodeId: 'skill:cluster-a-skill',
            relationType: 'mitigates',
            strength: 'hard',
            evidence: 'test',
          },
      ...
      makeDoc(
        ...
        1,
        [
          { id: 'trap:cluster-b-trap', kind: 'trap', label: 'Cluster B Trap', evidence: 'test' },
          {
            id: 'skill:cluster-b-skill',
            kind: 'skill',
            label: 'Cluster B Skill',
            evidence: 'test',
          },
        ],
        [
          {
            id: 'cluster-b-edge',
            sourceNodeId: 'trap:cluster-b-trap',
            targetNodeId: 'skill:cluster-b-skill',
            relationType: 'mitigates',
            strength: 'hard',
            evidence: 'test',
          },
...
function buildCycleDataset() {
  const nodes: GraphNodeRecord[] = ['a', 'b', 'c'].map((id) => ({
    id: `skill:${id}`,
    kind: 'skill',
    label: id,
    evidence: 'test',
  }));
  const makeCycle = (lastStrength: 'hard' | 'soft') =>
    makeDoc(`cycle-${lastStrength}`, 'skill', `cycle-${lastStrength}`, 1, nodes, [
      {
        id: 'a-b',
  ...
  return {
    mixedCycleDoc: makeCycle('soft'),
    hardCycleDoc: makeCycle('hard'),
    diamondDoc: makeDoc(
      'diamond',
      'skill',
      'diamond',
