...
describe('skill artifact derivation (CAPS-01, CAPS-02, CAPS-03)', () => {
  ...
  describe('applyDerivedArtifactOutputs() persistence', () => {
    it('should persist derived outputs on the revision record', async () => {
      ...
      const updatedArtifact = await applyDerivedArtifactOutputs(artifact, revision, derived);
...
//
// Documents the legacy revision behavior where a new revision always creates
...
//
// Owner-local write paths must decide whether derived data carries forward.
...
describe('Phase 1 regression: retrieval reads stale derived data after edit', () => {
  ...
  beforeEach(async () => {
    ...
    artifact = await applyDerivedArtifactOutputs(artifact, revision1, derived);
  ...
  it('should show latestRevision.derived is null after edit (current broken behavior)', () => {
    // Simulate an edit that creates a new revision:
