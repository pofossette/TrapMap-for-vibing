...
import { dependencyStatusSchema, healthStatusSchema } from '../../src/domain/health.js';
...
describe('C5 dependency summary / readiness schema', () => {
  it('accepts healthStatusSchema payloads unchanged (additive optional)', async () => {
    const { healthStatusSchema, readinessStatusSchema } = await import(
  ...
  it('validates dependencySummary bounds and enum states', async () => {
    const { readinessStatusSchema } = await import('../../src/domain/health.js');
    const base = {
      status: 'ok',
      timestamp: new Date().toISOString(),
      startedAt: new Date().toISOString(),
