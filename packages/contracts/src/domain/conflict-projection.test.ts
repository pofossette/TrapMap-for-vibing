...
import type { ConflictReadProjection } from './conflict-projection.js';
...
describe('conflict read projection', () => {
  it('returns only hints visible to the retrieval actor', () => {
    const hints = enrichConflictHints(
      ...
      [
        { id: 'entry-a', shortcut: 'Public option', teamId: null, requiredLevel: 0 },
        { id: 'entry-b', shortcut: 'Restricted option', teamId: 'team-b', requiredLevel: 7 },
      ],
  ...
  it('returns the related entry hint when the retrieval actor can view it', () => {
    const hints = enrichConflictHints(
      ...
      [
        { id: 'entry-a', shortcut: 'First option', teamId: null, requiredLevel: 0 },
        { id: 'entry-b', shortcut: 'Second option', teamId: null, requiredLevel: 5 },
      ],
      { teamId: 'team-a', requiredLevel: 5 },
    );

    expect(hints.get('entry-a')).toEqual([
      {
        entryId: 'entry-b',
        shortcut: 'Second option',
        conflictType: 'contradictory',
        context: 'Opposite instructions',
      },
    ]);
...
it('defines a knowledge-read-only conflict projection contract', async () => {
  const projection: ConflictReadProjection = {
    async listByEntryIds(entryIds) {
      return entryIds.length === 1 ? [] : [];
    },
  };

  await expect(projection.listByEntryIds(['entry-a'])).resolves.toEqual([]);
});
