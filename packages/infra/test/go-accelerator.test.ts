...
import { batchCosineWithFallback, canonicalHashWithFallback, cosineWithFallback } from '../src/go-accelerator/fallback.js';

describe('go-accelerator client fallback', () => {
  it('canonicalHash fallback when disabled', async () => {
    const client = new GoAcceleratorClient({ enabled: false, baseUrl: 'http://localhost:4100', timeoutMs: 1000 });
  ...
  it('batchCosine fallback when disabled', async () => {
    const client = new GoAcceleratorClient({ enabled: false, baseUrl: 'http://localhost:4100', timeoutMs: 1000 });
    const scores = await batchCosineWithFallback([1,0], [[1,0],[0,1]], client);
    expect(scores[0]).toBeGreaterThan(scores[1]);
  });

  it('uses Go when enabled and succeeds', async () => {
    const fetchMock = vi.fn(async () => new Response(JSON.stringify({ similarity: 0.9, normA: 1, normB: 1 }), { status: 200, headers: { 'Content-Type': 'application/json' } }));
    const client = new GoAcceleratorClient({ enabled: true, baseUrl: 'http://localhost:4100', timeoutMs: 1000, fetchImpl: fetchMock as unknown as typeof fetch });
  ...
  it('falls back on Go failure', async () => {
    ...
    const client = new GoAcceleratorClient({ enabled: true, baseUrl: 'http://localhost:4100', timeoutMs: 100, fetchImpl: fetchMock as unknown as typeof fetch });
