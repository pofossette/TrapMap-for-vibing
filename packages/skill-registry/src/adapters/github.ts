...
export class GithubAdapter implements RegistryAdapter {
  ...
  async fetchBundle(source: SkillSource): Promise<SkillBundle> {
    if (!source.owner || !source.repo) throw new Error(`GitHub source missing owner/repo: ${source.raw}`);
    ...
    for (const cand of candidates) {
      try {
        ...
        files.push({ path: cand, content, sha256: await sha256(content), sizeBytes: Buffer.byteLength(content) });
      } catch {
        continue;
      }
    }
