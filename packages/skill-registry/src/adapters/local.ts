  ...
  async fetchBundle(source: SkillSource): Promise<SkillBundle> {
    ...
    if (st.isDirectory()) {
      ...
      try {
        ...
        files.push({ path: 'SKILL.md', content, sha256: await sha256(content), sizeBytes: Buffer.byteLength(content) });
      } catch {
      ...
      try {
        ...
        for (const e of entries) {
          if (e.isFile()) {
            ...
            files.push({ path: `references/${e.name}`, content, sha256: await sha256(content), sizeBytes: Buffer.byteLength(content) });
          }
        }
      } catch {
        // no references is ok
      ...
      files.push({ path: name, content, sha256: await sha256(content), sizeBytes: Buffer.byteLength(content) });
    }
    return {
