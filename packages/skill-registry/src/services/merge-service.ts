...
export class MergeService {
  async check(baseDir: string, localDir: string, remoteBundle: { slug: string; version?: string; files: Array<{ path: string; content: string; sha256: string }> }): Promise<MergeCheckResult> {
    const base = await readSnapshot(baseDir, remoteBundle.slug);
    const local = await readSnapshot(localDir, remoteBundle.slug);
    const remote: SkillSnapshot = { slug: remoteBundle.slug, version: remoteBundle.version, files: remoteBundle.files.map((f) => ({ path: f.path, content: f.content, sha256: f.sha256 })) };
  ...
  async merge(
    baseDir: string,
    localDir: string,
    remoteBundle: { slug: string; version?: string; files: Array<{ path: string; content: string; sha256: string }> },
  ...
  async status(cwd: string = process.cwd(), scope: 'global' | 'project' = 'project'): Promise<Array<{ slug: string; hasLocalEdits: boolean; diff: ReturnType<typeof diffSnapshots> }>> {
    const baseRoot = scope === 'global' ? path.join(process.env.HOME ?? cwd, '.trapmap', 'skills') : path.join(cwd, '.trapmap', 'skills');
    try {
