...
export class InstallService {
  ...
  resolveInstallPaths(slug: string, options: InstallOptions): { primary: string; agentPaths: string[] } {
  ...
  async install(rawSource: string, options: InstallOptions = {}): Promise<InstallResult> {
    const bundle = await this.registry.fetchBundle(rawSource, options.overwrite ? undefined : undefined);
    ...
    return { slug: bundle.slug, version: bundle.version, installedPath: primary, filesWritten: bundle.files.length, lockEntry };
  }

  private async updateLockfile(slug: string, entry: SkillLockEntry, options: InstallOptions): Promise<void> {
    const cwd = options.cwd ?? process.cwd();
    const scope = options.scope ?? 'project';
    ...
    let lockfile: SkillLockfile = { version: 1, generatedAt: new Date().toISOString(), entries: {} };
    try {
  ...
  async installFromLockfile(options: InstallOptions = {}): Promise<InstallResult[]> {
    ...
    for (const [slug, entry] of Object.entries(lockfile.entries)) {
      ...
      const result = await this.install(bundle.source.raw, { ...options, agentTargets: entry.agentTargets, scope: entry.scope });
      results.push(result);
    }
    return results;
  }
