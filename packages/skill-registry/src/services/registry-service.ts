...
export class RegistryService {
  ...
  parseSource(raw: string): SkillSource {
    ...
    if (input.startsWith('.') || input.startsWith('/') || input.startsWith('~') || input.startsWith('file://')) {
      ...
      return skillSourceSchema.parse({ kind: 'local-path', raw: input, canonical, slug: canonical.split('/').pop() });
    }
    ...
    const githubMatch = input.match(/^(?:github:)?([A-Za-z0-9_.-]+)\/([A-Za-z0-9_.-]+)(?:\/([^@#]+))?(?:[@#](.+))?$/);
    ...
    if (ghUrl) {
      ...
      const canonical = `${owner}/${repo}${subpath ? `/${subpath}` : ''}`;
      return skillSourceSchema.parse({ kind: 'github', raw: input, canonical, owner, repo, subpath, ref });
    }
    ...
    return skillSourceSchema.parse({ kind: 'skills-sh', raw: input, canonical: input, slug: input });
  }
  ...
  async searchAll(query: RegistrySearchQuery): Promise<{ registry: string; entries: SkillRegistryEntry[] }[]> {
  ...
  async fetchBundle(rawSource: string, version?: string) {
    const source: SkillSource = version ? { ...this.parseSource(rawSource), version } : this.parseSource(rawSource);
