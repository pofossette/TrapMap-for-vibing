  ...
  for (const path of allPaths) {
    ...
    if (!b && l && !r) {
      ...
      if (l.sha256 === r.sha256) mergedFiles.push(l);
        ...
        conflicts.push({ path, localContent: l.content, remoteContent: r.content, reason: 'both added with different content' });
        if (strategy === 'ours') mergedFiles.push(l);
        else if (strategy === 'theirs') mergedFiles.push(r);
        else if (strategy === 'union') {
          mergedFiles.push(l);
          if (!mergedFiles.some((f) => f.path === r.path)) mergedFiles.push({ ...r, path: r.path + '.remote' });
        }
      }
    } else if (b && !l && !r) {
      ...
      if (!remoteChanged) {
        ...
        conflicts.push({ path, baseContent: b.content, remoteContent: r.content, reason: 'deleted locally but modified remotely' });
      ...
      if (!localChanged) {
        ...
        conflicts.push({ path, baseContent: b.content, localContent: l.content, reason: 'deleted remotely but modified locally' });
        if (strategy === 'ours') mergedFiles.push(l);
      ...
      if (!localChanged && !remoteChanged) mergedFiles.push(b);
        ...
        if (l.sha256 === r.sha256) mergedFiles.push(l);
          ...
          conflicts.push({ path, baseContent: b.content, localContent: l.content, remoteContent: r.content, reason: 'both modified divergently' });
          if (strategy === 'ours') mergedFiles.push(l);
          else if (strategy === 'theirs') mergedFiles.push(r);
          else if (strategy === 'union') {
