...
export function diffSnapshots(base: SkillSnapshot, next: SkillSnapshot): SkillDiff {
  ...
  for (const p of allPaths) {
    ...
    else if (b && n && b.sha256 !== n.sha256) files.push({ path: p, status: 'modified', baseHash: b.sha256, nextHash: n.sha256 });
    else if (b && n) files.push({ path: p, status: 'unchanged', baseHash: b.sha256, nextHash: n.sha256 });
  }
